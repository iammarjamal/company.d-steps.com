    ...
    <div @click.outside="popup = false" x-show="popup" x-transition class="absolute p-2 rtl:left-[-6vh] ltr:right-[-6vh] mt-4 w-64 xl:w-72 border border-zinc-200 dark:border-zinc-600 shadow-lg shadow-zinc-600/5 dark:shadow-zinc-400/5 bg-zinc-50 dark:bg-zinc-900 rounded-lg overflow-hidden z-10" x-cloak>
        <div class="h-48 p-2 overflow-auto">
            @forelse ($notifications as $notification)
                <a wire:click="read({{$notification->id}})" class="flex cursor-pointer items-center px-4 py-3 text-zinc-900 dark:text-zinc-50 hover:text-white hover:bg-indigo-600 hover:dark:bg-indigo-600 rounded-md -mx-2 @if($notification->status == 'unread') bg-zinc-900/5 dark:bg-zinc-50/5 @endif">
            ...
            <div class="flex items-center justify-center h-full text-center">
                <x-camelui::heading>لا يوجد إشعارات حالياً</x-camelui::heading>
            </div>
            @endforelse
        </div>
