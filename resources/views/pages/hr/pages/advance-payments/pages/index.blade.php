        ...
        @else
            <table class="w-full text-center text-sm text-slate-700 dark:text-slate-300">
                ...
                <tr>
                    ...
                    <th scope="col" class="p-4">الموظف</th>
                ...
                @foreach($advancePayments as $advancePayment)
                    <tr  wire:key="{{$advancePayment->id}}">
                        <td class="p-4">{{ $advancePayment->id }}</td>
                        <td class="p-4">{{ $advancePayment->user->username }}</td>
                        ...
                        <td x-data="{ update: false , remove: false  }"
                            ...
                            @if($advancePayment->status === 'requested')
                                <!-- Update modal -->
                                <x-camelui::link
                                    x-on:click="update = ! update;$wire.setToUpdate({{$advancePayment->id}})">
                                        ...
                                        >
                                            ...
                                            <div class="flex justify-center mb-6" x-show="vacation">
                                                <form wire:submit="update({{$advancePayment->id}})"
                                                    ...
                                                    <div class="relative w-full mb-3">
                                                        <x-camelui::input
                                                            label="{{ trans('app.advance-payment-value') }}"
                                                            type="number"
                                ...
                                <x-camelui::button style="success" class="py-1 text-sm" icon="fa-solid fa-check"
                                                   wire:click="approveAdvancePayment({{ $advancePayment->id }})"
                                                   wire:target="approveAdvancePayment({{ $advancePayment->id }})">
                                    موافقة
                                </x-camelui::button>
                            @endif
                        </td>
